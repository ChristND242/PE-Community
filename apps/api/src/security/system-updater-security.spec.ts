...
import {
  ...
  normalizeVersion,
} from '../system-updates/system-version';
...
test('system version metadata is normalized and rejects unsafe build values', () => {
  assert.deepEqual(
    ...
    }),
    {
      version: 'v1.2.3',
      sourceCommit: 'a'.repeat(40),
      buildDate: '2026-08-30T00:00:00.000Z',
      channel: 'stable',
    },
  );
  assert.deepEqual(
    currentSystemVersion({
      ...
      sourceCommit: 'a'.repeat(40),
      buildDate: '2026-09-02T19:29:32Z',
      channel: 'validation',
    }),
    {
      version: 'v0.0.0-validation.33673603612',
  ...
  assert.deepEqual(currentSystemVersion(), {
    version: 'v0.0.0-dev',
    sourceCommit: null,
    buildDate: null,
    channel: 'development',
...
test('release builds embed immutable version, source commit, and timestamp metadata', async () => {
  const [dockerfile, generator, workflow] = await Promise.all([
    ...
    readFile(
      new URL('../../scripts/write-build-metadata.mjs', import.meta.url),
      'utf8',
    ),
    readFile(
      new URL(
  ...
  assert.match(
    dockerfile,
    /node apps\/api\/scripts\/write-build-metadata\.mjs/,
  );
...
test('update execution is protected by permission and existing recent-auth step-up', async () => {
  const controller = await readFile(
    new URL('../system-updates/system-updates.controller.ts', import.meta.url),
    'utf8',
  );
  ...
  assert.doesNotMatch(
    controller,
    /currentPassword|passwordHash|verifyPassword/,
  );
});
...
test('automatic release discovery requires provenance contract while the agent remains authoritative', async () => {
  const source = await readFile(
    new URL('../system-updates/release-discovery.service.ts', import.meta.url),
    'utf8',
  );
  ...
  assert.doesNotMatch(
    source,
    /DIGEST_ONLY'\s*\?\s*SystemUpdateCheckStatus\.UPDATE_AVAILABLE/,
  );
});
...
test('updater IPC signatures bind protocol, operation, nonce, and request body', async () => {
  const [client, server] = await Promise.all([
    readFile(
      new URL('../system-updates/updater-agent.client.ts', import.meta.url),
      'utf8',
    ),
    readFile(
