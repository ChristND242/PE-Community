...
export type SystemVersion = {
  ...
  channel: 'stable' | 'development' | 'validation';
};

export function currentSystemVersion(
  metadata: SystemVersion = BUILD_METADATA,
): SystemVersion {
  ...
  if (metadata.channel === 'validation') {
    const version = strictValidationVersion(metadata.version);
    const sourceCommit = /^[a-f0-9]{40}$/.test(metadata.sourceCommit ?? '')
    ...
    return { version, sourceCommit, buildDate, channel: 'validation' };
  }
...
function strictValidationVersion(value: unknown) {
  return typeof value === 'string' &&
    /^v0\.0\.0-validation\.[1-9][0-9]*$/.test(value)
    ? value
    : null;
}
