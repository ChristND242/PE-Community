...
export type EventImageSelection = {
...
export type EventImageFieldPhase = 'EMPTY' | 'UPLOAD_SELECTING' | 'URL_ENTERING' | 'IMAGE_SELECTED';

export type EventImageFieldWorkflow = {
  phase: EventImageFieldPhase;
  replacing: boolean;
};

export type EventImageFieldAction = 'START_UPLOAD' | 'START_URL' | 'START_REPLACE' | 'CANCEL' | 'SELECT' | 'REMOVE';

export const eventImageAccept = 'image/jpeg,image/png,image/webp';
export const eventImageMaxBytes = 5 * 1024 * 1024;

type SupportedEventImageMime = 'image/jpeg' | 'image/png' | 'image/webp';

export function detectEventImageMime(bytes: Uint8Array): SupportedEventImageMime | null {
...
export async function normalizeEventImageFile(file: File, fallbackBasename = 'event-image') {
  const bytes = new Uint8Array(await file.slice(0, 12).arrayBuffer());
  const detectedMime = detectEventImageMime(bytes);
...
export function initialEventImageSelection(imageUrl?: string | null, imageSource?: 'UPLOAD' | 'EXTERNAL' | null): EventImageSelection {
  return {
...
export function validEventImageUrl(value: string) {
  try {
...
export function hasSelectedEventImage(value: EventImageSelection) {
  if (value.mode === 'UPLOAD') return Boolean(value.file || value.url);
  return value.mode === 'EXTERNAL' && validEventImageUrl(value.url);
}

export function initialEventImageFieldWorkflow(value: EventImageSelection): EventImageFieldWorkflow {
  return { phase: hasSelectedEventImage(value) ? 'IMAGE_SELECTED' : 'EMPTY', replacing: false };
}

export function transitionEventImageField(
  workflow: EventImageFieldWorkflow,
  action: EventImageFieldAction,
  hasSelectedImage: boolean,
): EventImageFieldWorkflow {
...
export function eventImageValidation(value: EventImageSelection, labels: { fileRequired: string; invalidUrl: string }) {
  if (value.mode === 'UPLOAD' && value.changed && !value.file) return labels.fileRequired;
  if (value.mode === 'EXTERNAL' && (!value.url.trim() || !validEventImageUrl(value.url))) return labels.invalidUrl;
  return null;
}

export function eventRequestBody(fields: Record<string, string>, image: EventImageSelection) {
  if (!image.changed) return JSON.stringify(fields);
  if (image.mode !== 'UPLOAD') return JSON.stringify({ ...fields, imageMode: image.mode, imageUrl: image.mode === 'EXTERNAL' ? image.url.trim() : undefined });
  const body = new FormData();
  Object.entries(fields).forEach(([key, value]) => body.append(key, value));
  body.append('imageMode', 'UPLOAD');
  if (image.file) body.append('eventImage', image.file);
  return body;
}

export type EventImageFieldLabels = {
  title: string;
  optional: string;
  description: string;
  noImage: string;
...
export function EventImageField({ value, onChange, labels, className, disabled = false, loading = false, errorMessage = '', urlPlaceholder = 'https://example.com/event.jpg', fallbackBasename = 'event-image' }: {
  value: EventImageSelection;
  onChange: (value: EventImageSelection) => void;
  labels: EventImageFieldLabels;
  className?: string;
  disabled?: boolean;
  loading?: boolean;
  errorMessage?: string;
  ...
  const [workflow, setWorkflow] = useState(() => initialEventImageFieldWorkflow(value));
  const [urlDraft, setUrlDraft] = useState('');
  const [objectUrl, setObjectUrl] = useState('');
  const [error, setError] = useState('');
  const [previewFailed, setPreviewFailed] = useState(false);
  const [checkingFile, setCheckingFile] = useState(false);
  const selected = hasSelectedEventImage(value);
  ...
  useEffect(() => {
    ...
    setWorkflow((current) => {
      if (selected && current.phase === 'EMPTY') return initialEventImageFieldWorkflow(value);
      if (!selected && current.phase === 'IMAGE_SELECTED') return initialEventImageFieldWorkflow(value);
      return current;
    });
  }, [selected, value.file, value.mode, value.url]);

  const externalPreview = value.mode === 'EXTERNAL' && validEventImageUrl(value.url) ? value.url.trim() : '';
  ...
  function move(action: EventImageFieldAction) {
    setError('');
    setWorkflow((current) => transitionEventImageField(current, action, selected));
  }
  ...
  async function chooseFile(file?: File) {
    if (!file) return;
    if (file.size > eventImageMaxBytes) {
    ...
    try {
      const normalizedFile = await normalizeEventImageFile(file, fallbackBasename);
      if (!normalizedFile) {
      ...
      {workflow.phase === 'UPLOAD_SELECTING' && (
        <div className="grid overflow-hidden rounded-xl border border-[var(--app-border)] bg-[var(--app-panel-muted)] sm:grid-cols-[minmax(0,1fr)_minmax(16rem,1fr)]">
          ...
          <div className="flex flex-col justify-center gap-3 border-t border-[var(--app-border)] p-4 sm:border-l sm:border-t-0">
            ...
            <input ref={inputRef} type="file" accept={eventImageAccept} className="sr-only" onChange={(event) => { void chooseFile(event.target.files?.[0]); }} />
            <div className="flex flex-wrap gap-2">
...
export const ImageSelectionField = EventImageField;
export type ImageSelection = EventImageSelection;
export type ImageSelectionFieldLabels = EventImageFieldLabels;

function CurrentImageSummary({ previewUrl, previewFailed, labels, onPreviewFailed }: { previewUrl: string; previewFailed: boolean; labels: EventImageFieldLabels; onPreviewFailed: () => void }) {
  return (
...
function SelectedImagePreview({ previewUrl, previewFailed, labels, actionClassName, onPreviewFailed, onReplace, onRemove }: { previewUrl: string; previewFailed: boolean; labels: EventImageFieldLabels; actionClassName: string; onPreviewFailed: () => void; onReplace: () => void; onRemove: () => void }) {
  return (
