...
test('system update UI keeps execution behind step-up and confirmation', async () => {
  const source = await readFile(new URL('./system-updates-settings.tsx', import.meta.url), 'utf8');
  assert.match(source, /stepUp\.run\(\(\) => apiFetch\(`\/admin\/\$\{COMMUNITY_ID\}\/system-updates\/authorize/);
...
test('history, realtime logs, and update badge remain permission and state scoped', async () => {
  const [settings, badge, socket, shell, settingsPage] = await Promise.all([
    ...
    readFile(new URL('../hooks/use-system-update-badge.ts', import.meta.url), 'utf8'),
    readFile(new URL('../hooks/use-system-update-socket.ts', import.meta.url), 'utf8'),
...
test('empty release catalogs and development builds have calm non-installable UI states', async () => {
  ...
  assert.match(settings, /release\.status === 'UPDATE_AVAILABLE' && <LoadingButton/);
...
test('supply-chain status is display-only and has no provenance bypass control', async () => {
  ...
  assert.doesNotMatch(source, /continueAnyway|bypassProvenance|skipProvenance/i);
});
