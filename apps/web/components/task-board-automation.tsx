...
function ruleStatus(rule: Rule, t: ReturnType<typeof useI18n>['t'], executionStopped = false) { if (executionStopped) return { label: t.admin.automationStatusStopped, tone: 'border-amber-200/15 bg-amber-300/[0.05] text-amber-100/75' }; if (!rule.enabled) return { label: t.admin.automationStatusDisabled, tone: 'border-white/10 bg-white/[0.04] text-white/40' }; if (rule.lastRunStatus === 'SUCCESS') return { label: t.admin.automationSuccess, tone: 'border-accent/20 bg-accent/10 text-accent' }; if (rule.lastRunStatus === 'FAILED') return { label: t.admin.automationFailed, tone: 'border-rose-200/20 bg-rose-300/[0.06] text-rose-100' }; if (rule.lastRunStatus === 'SKIPPED') return { label: t.admin.automationSkipped, tone: 'border-[var(--app-warning-border)] bg-[var(--app-warning-soft)] text-[var(--app-warning-foreground)]' }; if (isNotificationRule(rule.type)) return { label: t.admin.automationStatusScheduled, tone: 'border-sky-200/15 bg-sky-300/[0.07] text-sky-100/75' }; return { label: t.admin.automationNeverRun, tone: 'border-white/10 bg-white/[0.04] text-white/45' }; }
