...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number; name?: string; payload?: ChartDatum }>; label?: string }) {
  ...
  return (
    <div className="rounded-lg border border-[var(--app-border)] bg-[var(--app-elevated)] px-3 py-2 text-xs shadow-2xl shadow-black/20">
      <p className="font-semibold text-[var(--app-foreground)]">{item.payload?.tooltipLabel ?? item.payload?.label ?? label ?? item.name}</p>
    ...
    <div style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 8, right: 10, left: -20, bottom: 0 }}>
          ...
          <XAxis dataKey="label" tick={{ fill: 'var(--chart-axis)', fontSize: 11 }} axisLine={false} tickLine={false} interval={0} />
