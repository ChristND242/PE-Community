...
function AuditDetailPane({ detail, locale, timezone, t }: { detail: AuditDetail; locale: string; timezone: string; t: ReturnType<typeof useI18n>['t'] }) {
  return <div className="space-y-6">
    ...
    <DetailSection title={t.admin.auditLogsActor}><DetailGrid rows={[[t.admin.auditLogsName, detail.actor.name], [t.admin.auditLogsEmail, detail.actor.email], [t.admin.auditLogsActorType, detail.actor.type], [t.admin.auditLogsRecordedRole, detail.actor.recordedRole], [t.admin.auditLogsCurrentRole, detail.actor.currentRole]]} /></DetailSection>
...
function DetailGrid({ rows, mono = false }: { rows: Array<[string, string | null | undefined]>; mono?: boolean }) { return <dl className="grid gap-x-5 gap-y-3 sm:grid-cols-[9rem_minmax(0,1fr)]">{rows.filter(([, value]) => value).map(([label, value]) => <div key={label} className="contents"><dt className="text-sm text-white/42">{label}</dt><dd className={`break-words text-sm text-white/72 ${mono ? 'font-mono text-xs' : ''}`}>{value}</dd></div>)}</dl>; }
