...
export function EventTaskBoard({ eventId, canManage, canArchive, minimalHeading = false }: { eventId: string; canManage: boolean; canArchive: boolean; minimalHeading?: boolean }) {
  ...
  const taskDialogRootRef = useRef<HTMLDivElement>(null);
  const taskDialogBoundaryRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <section aria-labelledby="event-task-board-title" className="space-y-4">
      ...
      {formOpen && typeof document !== 'undefined' && createPortal(
        <div ref={taskDialogRootRef} data-task-board-dialog-root className="fixed inset-0 z-[70] grid h-dvh place-items-center overflow-y-auto p-4">
          <div data-task-board-dialog-overlay aria-hidden="true" className="fixed inset-0 z-0 bg-black/70" />
          <div ref={taskDialogBoundaryRef} className="relative z-10 my-6 max-h-[calc(100dvh-3rem)] w-full max-w-3xl overflow-y-auto rounded-xl border border-white/10 bg-[#0a120f] p-5 shadow-2xl shadow-black/50 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="event-task-form-title">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <AppSelect value={form.priority} label={t.admin.eventTaskPriority} options={(['LOW', 'MEDIUM', 'HIGH'] as EventTaskPriority[]).map((priority) => ({ value: priority, label: priorityName(t, priority) }))} containedPositioning={{ boundaryRef: taskDialogBoundaryRef, portalRef: taskDialogRootRef }} onChange={(priority) => setForm({ ...form, priority })} />
              <fieldset className="rounded-xl border border-white/10 bg-black/20 p-3"><legend className="px-1 text-sm text-white/70">{t.admin.eventTaskAssignee}</legend><div className="mt-1 max-h-36 space-y-1 overflow-y-auto">{members.map((member) => { const checked = form.assigneeIds.includes(member.user.id); return <label key={member.user.id} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-white/65 hover:bg-white/[0.05]"><input type="checkbox" checked={checked} onChange={() => setForm({ ...form, assigneeIds: checked ? form.assigneeIds.filter((id) => id !== member.user.id) : [...form.assigneeIds, member.user.id] })} className="accent-emerald-400" /><span className="min-w-0 truncate">{member.user.name}</span><IdentityVerificationBadge kind={identityVerificationForRole(member.role?.key)} size="xs" /></label>; })}{members.length === 0 && <p className="px-2 py-2 text-xs text-white/35">{t.admin.eventTaskUnassigned}</p>}</div></fieldset>
