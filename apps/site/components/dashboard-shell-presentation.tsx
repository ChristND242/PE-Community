...
export function DashboardDesktopSidebar({ workspaceName, roleLabel, children, contained = false, brandHref }: { workspaceName: string; roleLabel?: string; children: React.ReactNode; contained?: boolean; brandHref?: string }) {
  ...
  return (
    <aside className={`site-preview-sidebar ${contained ? 'absolute' : 'fixed'} inset-y-0 left-0 hidden w-72 flex-col overflow-y-auto border-r border-white/10 bg-[#070b0a]/95 p-5 shadow-2xl shadow-black/30 backdrop-blur lg:flex`}>
      {brandHref
...
export function DashboardTopbar({ workspaceName, user, desktopControls, mobileControls, menuOpen, onOpenMenu }: { workspaceName: string; user: ShellCurrentUser | null; desktopControls: React.ReactNode; mobileControls?: React.ReactNode; menuOpen: boolean; onOpenMenu: () => void }) {
  ...
  return (
    <header className="site-preview-header sticky top-0 z-20 flex items-center justify-between gap-4 border-b border-white/10 bg-background/82 px-4 py-3 backdrop-blur-xl sm:px-6">
...
export function DashboardSidebarNavItem({ label, icon: Icon, active, badgeCount = 0, href, onClick, ariaLabel, mobile = false }: { label: string; icon: LucideIcon; active: boolean; badgeCount?: number; href?: string; onClick?: () => void; ariaLabel?: string; mobile?: boolean }) {
  const tone = mobile
    ? active ? 'border-accent/20 bg-accent/15 text-accent' : 'border-transparent text-white/68 hover:border-white/10 hover:bg-white/[0.055] hover:text-white'
    : active ? 'border-accent/20 bg-accent/15 text-accent shadow-lg shadow-accent/5' : 'border-transparent text-white/62 hover:border-white/10 hover:bg-white/[0.055] hover:text-white';
  const className = `group flex w-full items-center gap-3 rounded-xl border px-3 text-left text-sm transition ${mobile ? 'py-3' : 'py-2.5'} ${tone}`;
