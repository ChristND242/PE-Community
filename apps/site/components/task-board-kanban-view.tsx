...
export function TaskBoardKanbanView({ dndContextId, tasks, members, locale, canManage, moving, activeTask, onDragStart, onDragOver, onDragEnd, onDragCancel, onEdit, onComments, onMove, t }: TaskBoardKanbanViewProps) {
  ...
  return (
    <DndContext id={dndContextId} sensors={sensors} collisionDetection={closestCorners} onDragStart={onDragStart} onDragOver={onDragOver} onDragEnd={onDragEnd} onDragCancel={onDragCancel}>
      <div className="chat-scrollbar overflow-x-auto pb-2">
...
function TaskColumn({ status, tasks, children, t }: { status: TaskBoardStatus; tasks: TaskBoardTask[]; children: ReactNode; t: ReturnType<typeof useI18n>['t'] }) {
  ...
  return <div ref={setNodeRef} className={`min-w-0 rounded-xl border p-3 transition-colors ${isOver ? 'border-accent/30 bg-accent/[0.045]' : 'border-white/10 bg-white/[0.025]'}`}><div className="flex items-center justify-between gap-3 px-1 py-1"><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${statusTone(status)}`} /><h3 className="text-sm font-semibold text-white">{statusName(t, status)}</h3></div><span className="rounded-full bg-white/[0.07] px-2 py-0.5 text-xs font-semibold tabular-nums text-white/55">{tasks.length}</span></div><SortableContext items={tasks.map((task) => task.id)} strategy={verticalListSortingStrategy}><div className="mt-3 min-h-28 space-y-3">{tasks.length === 0 ? <div className={`rounded-lg border border-dashed px-4 py-8 text-center transition-colors ${isOver ? 'border-accent/25 bg-accent/[0.035]' : 'border-white/10'}`}><ClipboardList className="mx-auto text-white/25" size={20} /><p className="mt-2 text-xs text-white/40">{isOver ? t.admin.eventTaskDropHere : t.admin.noEventTasks}</p></div> : children}</div></SortableContext></div>;
}
