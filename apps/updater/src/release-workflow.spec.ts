...
test('release workflow is SHA-pinned, least-privilege, draft-first, and inventory-gated', async () => {
  ...
  assert.match(workflow, /attestationPolicy:\\?"GITHUB_PROVENANCE_REQUIRED\\?"/);
  ...
  const imageJob = workflow.slice(workflow.indexOf('  image:'), workflow.indexOf('  publish:'));
  ...
  for (const dockerfilePath of ['../../api/Dockerfile', '../../web/Dockerfile', '../../worker/Dockerfile']) {
    ...
    assert.match(dockerfile, /org\.opencontainers\.image\.revision=\$\{SOURCE_COMMIT\}/);
    assert.match(dockerfile, /org\.opencontainers\.image\.created=\$\{BUILD_DATE\}/);
  }
});
...
test('workflow-shaped manifest fixture satisfies release contract version one', () => {
  ...
  const manifest = validateManifest({
    ...
    images: {
      api: { repository: 'ghcr.io/pona-ekolo/pe-community-api', digest: `sha256:${'a'.repeat(64)}` },
      web: { repository: 'ghcr.io/pona-ekolo/pe-community-web', digest: `sha256:${'b'.repeat(64)}` },
      worker: { repository: 'ghcr.io/pona-ekolo/pe-community-worker', digest: `sha256:${'c'.repeat(64)}` },
    },
