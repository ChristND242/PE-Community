...
- The agent verifies the bounded raw manifest file with the packaged `<updater-root>/bin/gh attestation verify` before parsing or trusting deployment fields. The path is derived from the updater package itself, never from `PATH`. Repository, workflow, SLSA predicate, tag ref, and independently resolved source commit are compiled policy. Tampering, missing provenance, invalid identity, network failure, timeout, or unsupported contract stops before image pull.
- Each pulled `RepoDigest` is checked before the packaged `<updater-root>/bin/gh attestation verify` validates the exact OCI digest against the compiled repository, signer workflow, SLSA predicate, tag ref, and source commit. Any verifier, network, output, identity, signature, workflow, digest, or commit failure stops before migration and deployment.
