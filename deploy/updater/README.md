...
The workflow extracts `gh` from the generated amd64 updater archive and performs two separate checks. First, a CI-only policy verifies the current validation image attestations against the current `main` source identity. Second, the same bundled verifier downloads the selected release's trusted manifest, verifies its manifest attestation, obtains its fixed allowlisted image digests, and verifies all three historical image attestations using the unchanged production tag-bound policy. The selected release must be published, stable, and satisfy the updater release contract; older releases that do not satisfy that contract fail closed.
